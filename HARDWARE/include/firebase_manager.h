...
public:
    ...
    void loop() {
        ...
        if (Firebase.getString(fbData, "/parking_status/cloud_command/cmd")) {
            ...
            if (cmd != "" && cmd != "IDLE") {
                ...
                if (Firebase.getString(fbData, "/parking_status/cloud_command/uid")) uid = fbData.stringData();
                if (Firebase.getString(fbData, "/parking_status/cloud_command/action")) action = fbData.stringData();
    ...
    bool checkRemoteWiFiChange(String &newSSID, String &newPass) {
        ...
        if (Firebase.getString(fbNetData, path + "/status")) {
            if (fbNetData.stringData() == "SWITCHING") {
                if (Firebase.getString(fbNetData, path + "/ssid")) newSSID = fbNetData.stringData();
                if (Firebase.getString(fbNetData, path + "/password")) newPass = fbNetData.stringData();
                return true;
            }
        }
        return false;
    ...
    void scanAndUploadNetworks() {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (rssi >= -50) net.set("strength", "Excellent");
            else if (rssi >= -65) net.set("strength", "Good");
            else if (rssi >= -75) net.set("strength", "Fair");
            else net.set("strength", "Weak");
