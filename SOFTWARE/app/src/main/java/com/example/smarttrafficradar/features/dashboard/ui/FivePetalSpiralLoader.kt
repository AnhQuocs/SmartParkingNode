...
@Composable
fun FivePetalSpiralLoader(modifier: Modifier = Modifier) {
    ...
    val pulse by infiniteTransition.animateFloat(
        initialValue = 0f, targetValue = 1f,
        animationSpec = infiniteRepeatable(
            tween(durationMillis = 4200, easing = LinearEasing),
            RepeatMode.Restart
        ),
        label = "pulse"
    )
    val rotation by infiniteTransition.animateFloat(
        ...
        animationSpec = infiniteRepeatable(
            tween(durationMillis = 28000, easing = LinearEasing),
            RepeatMode.Restart
        ),
        label = "rotation"
    )
    ...
    Canvas(modifier = modifier
        ...
        drawPath(
            path, Color.White.copy(alpha = 0.1f), style = Stroke(
                4.4f.dp.toPx(),
                cap = StrokeCap.Round
            )
        )
